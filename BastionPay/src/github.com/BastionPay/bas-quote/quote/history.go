...
//读取历史
func (this *HistoryMgr) Init(redis *db.Redis) (err error) {
	if redis == nil {
...
func (this *HistoryMgr) Set(tickerInfos *collect.TickerInfo, coin string) (gerr error) {
	...
	for i := 0; i < len(tickerInfos.IdDetailInfos); i++ {
		...
		nowDayTime := this.GenDay(IdDetailInfo.GetLast_updated())
		...
		for my, moneyInfo := range IdDetailInfo.Quotes {
			...
			ZapLog().With(zap.String("key", key), zap.Bool("listPush", updateWay)).Info("kxian set")
...
func (this *HistoryMgr) GenDay(t int64) int64 {
