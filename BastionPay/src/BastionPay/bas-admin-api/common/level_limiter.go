...
func DoLevelLimiter(redis *Redis, pls []*LevelPathLimit, ctx iris.Context, limitName, account string, level int) (bool, int, int, error) {
	return NewLevelLimiter(redis, pls, ctx, limitName, account, level).Verification()
}

//获取剩余的次数
func GetAllLevelRemainLimits(redis *Redis, pls []*LevelPathLimit, account string, level int) (map[string][]*LevelPathLimitRemain, error) {
	limiter := NewLevelLimiter(redis, pls, nil, "", account, level)
	return limiter.GetAllMatchLevelRemainLimits()
}

func NewLevelLimiter(redis *Redis, pls []*LevelPathLimit, ctx iris.Context, limitName, account string, level int) *LevelLimiter {
