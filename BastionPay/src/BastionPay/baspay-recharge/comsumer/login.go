...
func (l Login) LoginBastionPay(phone, pwd string) (string, string, error) {
	...
	if err != nil {
		...
		return "get token err", "refresh token err", err
	}
...
`)
	...
	bastionPayRes, err := base.HttpSendSer(config.GConfig.BastionpayUrl.Bastionurl+clientRsaUrl, bytes.NewBuffer(encyBody), "POST", map[string]string{"Client": "1", "DeviceType": "1", "DeviceName": "huawei", "DeviceId": config.GConfig.Login.DeviceId, "Version": "1.1.0", "Content-Type": "application/json;charset=UTF-8"}) //
...
func SendToBastion(body []byte, url string) (*ResEncyData, error) {
	...
	bastionPayRes, err := base.HttpSendSer(config.GConfig.BastionpayUrl.Bastionurl+url, bytes.NewBuffer(body), "POST", map[string]string{"Client": "1", "DeviceType": "1", "DeviceName": "huawei", "DeviceId": config.GConfig.Login.DeviceId, "Version": "1.1.0", "Content-Type": "application/json;charset=UTF-8"})
	if err != nil {
