...
func (this *LogLogin) ParseAdd(userId int, ip, country, city, device string) *LogLogin {
	this.UserId = &userId
	this.Ip = &ip
...
func (this *LogLogin) List(userId uint, limit int, skip int) ([]*LogLogin, int, error){
	...
	err = db.GDbMgr.Get().Model(&LogLogin{}).Where("user_id = ? AND created_at > ?", userId, time).
