...
func JwtSign(key string, minuteString string, userId uint, safe bool, email bool, phone bool, ga bool, uuid string, vipLevel uint8) (string, int64, error) {
	jwt, err := NewJwt(key, minuteString)
	if err != nil {
		return "", 0, err
	}
	token, err := jwt.Sign(userId, safe, email, phone, ga, uuid, vipLevel)
	if err != nil {
		return "", 0, err
	}
...
func (j *Jwt) Sign(userId uint, safe bool, email bool, phone bool, ga bool, uuid string, vipLevel uint8) (string, error) {
	claims := AppClaims{
		userId,
		safe,
		email,
