...
func (v *Verification) save() error {
	...
	ttl, err := GRedis.Do("TTL", VerificationPrefix+v.Id)
	if err != nil {
	...
	_, err = GRedis.Do("SET", VerificationPrefix+v.Id, string(b), "EX", expire)
	if err != nil {
		return err
	}
...
func (v *Verification) read() error {
	var err error
	result, err := GRedis.Do("GET", VerificationPrefix+v.Id)
	if err != nil {
