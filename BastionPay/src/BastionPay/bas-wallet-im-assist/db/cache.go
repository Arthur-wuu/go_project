...
func (this *Cache) Init() {
	GCache.AccountCache = gcache.New(config.GConfig.Cache.AccountMaxKey).LRU().Expiration(time.Duration(config.GConfig.Cache.AccountTimeout) * time.Second).Build()
}
...
func (this *Cache) SetAccountCache1(f func(key interface{}) (interface{}, *time.Duration, error)) {
	this.AccountCache = gcache.New(config.GConfig.Cache.AccountMaxKey).LRU().LoaderExpireFunc(f).Build()
}

func (this *Cache) SetAccountCache(account, statu string) {
	this.AccountCache.SetWithExpire(account, statu, time.Second*time.Duration(config.GConfig.Cache.AccountTimeout))
}
