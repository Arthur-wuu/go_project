...
func (this * TQueue) InQueue (data Ite){
	this.Items = append(this.Items, data)
}

func (this * TQueue) OutQueue () *Ite  {
	if len(this.Items) == 0 {
		return nil
	}
	d := this.Items[0]
...
func (this * TQueue) AddData () {
	seed := time.Now().Unix()
	data := rand.New(rand.NewSource(seed)).Intn(10)
	this.InQueue(data)
}

//func ChannelPrint ( intChannel chan int  , que TQueue) {
//	s := que.OutQueue()
//	a := *s
//	intChannel <- a.(int)
//}
...
func ChannelPrint1 ( que *TQueue) {
	s := que.OutQueue()
	if s == nil {
		fmt.Printf("1号服务 暂时等不到人\n")
		return
	}
	a := *s
	fmt.Printf("1号服务于 ： [%d] 客户 \n", a)
}

func ChannelPrint2 ( que *TQueue) {
	s := que.OutQueue()
	if s == nil {
		fmt.Printf("2号服务 暂时等不到人\n")
		return
	}
	a := *s
	fmt.Printf("2号服务于 ： [%d] 客户 \n", a)
}
...
func main()  {
	queue := NewTQueue()
	//channl1 := make(chan int,20)
	//channl2 := make(chan int,20)
