...
type RecordListParamsRuoXi struct {
...
//企业ID先定义成常量，以后会有调整
const CORPIDRUOXI = "dingf73d6ade3659feaa35c2f4657eb6378f"
const APPKEYRUOXI = "ding5za7gwojjva0xsby"
const APPSECRETRUOXI = "XBVhY9a7gf1kqZHtlY0lwjvAYOCp7XQXEB0CnRW6OrbRnJ9ZrGnJ93fjHlGBnkE6"
const CORPPREFIX = "ruoxi_"

func NewRuoXi() *DingtalkRuoXi {
	return &DingtalkRuoXi{
		AppKey:        APPKEYRUOXI,
		AppSecret:     APPSECRETRUOXI,
		AppAuthAccess: "",
		Host:          config.GConfig.Dingding.Host,
	}
}

func (this *DingtalkRuoXi) SetAuthAccess() {
	...
	if ret.ExpiresIn == nil {
		db.GCache.Dingtalk.SetWithExpire(CORPPREFIX+config.GConfig.Gcache.SecretKey, ret.AccessToken, time.Duration(*ret.ExpiresIn)*time.Second)
	} else {
		db.GCache.Dingtalk.SetWithExpire(CORPPREFIX+config.GConfig.Gcache.SecretKey, ret.AccessToken, time.Duration(config.GConfig.Gcache.Expire)*time.Second)
	}
}

func (this *DingtalkRuoXi) GetAuthAccess() string {
	token := ""
...
func (this *DingtalkRuoXi) GetDingdingRecordList() (*[]api.Recordresult, error) {
	//get all account info
	acts := new(models.AccountMap).GetAllAccounts(CORPIDRUOXI)
...
func (this *DingtalkRuoXi) AddCheckinRecord() {
...
func (this *DingtalkRuoXi) AttenRewardSend() {
	recordResult, err := this.GetDingdingRecordList()
...
func (this *DingtalkRuoXi) OvertimeAwardSend() {
	ckModel := new(models.CheckinRecord)
	rList, err := ckModel.GetValidWorkOvertimeList(CORPIDRUOXI)
...
func (this *DingtalkRuoXi) RecordListPost(userIds []string, sTime, eTime string) (resp *http.Response, err error) {
 ...
 **/
func (this *DingtalkRuoXi) SendAward(rcd *models.CheckinRecord, bTime int64, coinBase float32, symbol string, coin decimal.Decimal, atype int) {
