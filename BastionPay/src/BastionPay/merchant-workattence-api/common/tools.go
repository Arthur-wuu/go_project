...
func (t *Tools) DatetimeToTimestamp(s string) int64 {
	tm, _ := time.ParseInLocation("2006-01-02 15:04:05", s, time.FixedZone("UTC", 8*3600))
	return tm.Unix()
}

func (t *Tools) DayBeginTimestamp() int64 {
	loc := time.FixedZone("UTC", 8*3600)
	timeStr := time.Now().In(loc).Format("2006-01-02")
	tsp, _ := time.ParseInLocation("2006-01-02 15:04:05", timeStr+" 00:00:00", loc)
	return tsp.Unix()
}

func (t *Tools) NowDataString() string {
	loc := time.FixedZone("UTC", 8*3600)
	timeStr := time.Now().In(loc).Format("2006-01-02")
	return timeStr
}

func (t *Tools) DatetimeToDayBeginTimestamp(s string) int64 {
	loc := time.FixedZone("UTC", 8*3600)
	tm, _ := time.ParseInLocation("2006-01-02 15:04:05", s, loc)
	timeStr := tm.In(loc).Format("2006-01-02")
	tsp, _ := time.ParseInLocation("2006-01-02", timeStr, loc)
	return tsp.Unix()
}

func (t *Tools) DateToDayBeginTimestamp(s string) int64 {
	tm, _ := time.ParseInLocation("2006-01-02", s, time.FixedZone("UTC", 8*3600))
	return tm.Unix()
}
